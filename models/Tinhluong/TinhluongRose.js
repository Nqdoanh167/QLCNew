...
TinhluongRoseSchema.pre('save', async function(next) {
    try{
        let maxId = await connection.model("TinhluongRose", TinhluongRoseSchema).find({},
            {ro_id:1}).sort({ro_id:-1}).limit(1);
        if(maxId && maxId.length){
            console.log(maxId)
            maxId = maxId[0].ro_id + 1;
            await Counter.findOneAndUpdate({TableId: 'TinhluongRoseId'}, {$set:{Count:maxId}});
