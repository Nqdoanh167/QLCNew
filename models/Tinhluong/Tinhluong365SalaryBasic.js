...
Tinhluong365SalaryBasicSchema.pre('save', async function(next) {
    try{
        let maxId = await connection.model("Tinhluong365SalaryBasic", Tinhluong365SalaryBasicSchema).find({},
            {sb_id:1}).sort({sb_id:-1}).limit(1);
        if(maxId && maxId.length){
            maxId = maxId[0].sb_id + 1;
            await Counter.findOneAndUpdate({TableId: 'Tinhluong365SalaryBasicId'}, {$set:{Count:maxId}});
